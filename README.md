...
CI runs a matrix of execution-guard checks, frontend smoke and regression/hardening gates through PR06.9.13, the PR08 frontend baseline jobs, the PR09 Ada-emission slice/baseline jobs, the PR10 emitted-output GNATprove contract/flow/prove/baseline jobs, the supplemental emitted hardening regression job, the PR10.1 comprehensive audit job, the PR11.1 language-evaluation harness job, and the SPARK companion plus emission-template verification jobs.
...
That tracked hook runs [`scripts/run_local_pre_push.py`](scripts/run_local_pre_push.py), which maps known `codex/pr08...`, `codex/pr09...`, and `codex/pr10...` branches, plus `codex/pr111...` branches, to the appropriate milestone gate plus the downstream evidence-refresh chain, then requires `git diff --exit-code` to remain clean. Unknown milestone branches fail closed until the mapping is updated.
