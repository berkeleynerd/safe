...
├── tests/                       # 120 test files (5 categories)
...
The repository now includes a sequential compiler frontend under `compiler_impl/`. The current compiler frontend supports the exact current Rule 5 fixture corpus, sequential ownership, and the current boolean result-record discriminant pattern. PR08.2 additionally brings the accepted local-only concurrency check surface for single-package task/channel/select/delay sources. It provides Ada-native `safec lex`, `ast`, `validate-mir`, `analyze-mir`, `check`, and `emit` for that supported surface, while Python remains glue/orchestration only around the compiler. The old shallow legacy frontend chain is gone, and PR07 is the milestone that establishes this expanded baseline before PR08.
