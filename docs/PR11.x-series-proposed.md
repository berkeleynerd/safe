...
Close the larger discriminant/type-system gap after the first parser-completion
pass has landed. This is not just parser work: it extends parsing, resolution,
type modeling, MIR lowering, and Ada emission for a bounded discriminated-record
subset that is materially larger than the current boolean result-record pattern.
...
- MIR lowering and emitted Ada support for the accepted surface.
...
- Emitted-structure assertions so the milestone cannot be satisfied by silently
  flattening or erasing discriminant semantics.
- Rosetta/sample updates once PR11.1 exists, using real examples that depend on
  discriminated records rather than synthetic micro-fixtures only.
...
- If the scope proves too large, the first split should be variant parts; the
  rest of the discriminant/core-constraint subset belongs together.
...
- Prove the emitted fixtures admitted by `PR11.2` string/character literal and
  case-statement support.
- Prove the emitted fixtures admitted by `PR11.3` discriminated-record and
  discriminant-constraint support.
- Revalidate any previously proved sequential fixtures whose emitted proof
  shape is affected by parser, resolver, or discriminant-emission changes.

### Deliverables

- A dedicated deterministic proof gate/report/CI job with a non-shrinkable
  named corpus covering the newly admitted `PR11.2` and `PR11.3` emitted
  fixture set.
...
- Matrix/tracker/docs wording that distinguishes the completed `PR10.x`
  sequential proof set from the `PR11.3a` parser/discriminant proof expansion.

### PS-029 Decision Point

`PS-029` should no longer be treated as a purely post-`PR11.11` concern. If
the admitted `PR11.3` discriminant corpus forces broader scope-exit or
deallocation-ordering semantics, those semantics must be resolved or explicitly
bounded before `PR11.3a` claims proof closure. If the accepted subset keeps
access-bearing deallocation cases out of scope, that narrower boundary should
be stated explicitly instead of implied.
...
- Parser support for the new spellings, unified `function` keyword, and `to`
  range keyword, with a clear coexistence or migration rule for legacy forms.
- Mechanical migration strategy for the existing test corpus and Rosetta corpus,
  including `procedure` → `function` conversion and `..` → `to` replacement.
- Deterministic acceptance corpus showing that the features are surface-only and
  do not perturb typing, MIR, or emission for already-supported programs.
...
general discriminants and discriminant constraints as a separate, larger
