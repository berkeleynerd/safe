...
- PR11.8h follows PR11.8g.1 (safe prove CLI command).
- PR11.9 follows PR11.8h.
...
- growable -> fixed narrowing is currently static-only:
  bracket literals and static name-based slices
- string iteration, string `case`, string ordering, and proof-based exact
  length narrowing are deferred to `PR11.8f`
...
- Array iteration is currently `for item of values` for array object names
  only. String iteration remains deferred.
...
- The following items are intentionally deferred beyond PR11.8d:
  string iteration, proof-based exact-length narrowing, string `case`, and
  string ordering to `PR11.8f`; string/growable channel elements to
  `PR11.8g`; string discriminants remain deferred.
...
### Also includes (deferred from PR11.8e)

- **Field-disjoint mut alias reasoning**: PR11.8e uses a conservative same-root
  alias rule — a call is rejected whenever two actuals share the same root object
  name. PR11.8e.1 may relax this to field-disjoint reasoning (two `mut` actuals
  for distinct non-overlapping fields of the same root are permitted). Requires
  alias analysis of field access paths; deferred to avoid unbounded scope in PR11.8e.
...
- **Negative fixture coverage bookkeeping**: PR11.8e's removed-syntax and
  ...
  ownership/task fixtures). Dedicated `neg_pr118e_*` fixtures now cover the
  genuinely PR11.8e-specific gaps: mutually recursive record-family rejection,
  recursive non-record shapes, inferred reference-bearing channel elements,
  `null` on pure values, and direct task-global read/write alongside the
  existing `.length` attribute case.
...
Follows PR11.8g.1. Must land before PR11.9 so that the artifact contract freeze
includes a stable proof-verification surface for developers.
...
- `PR11.8g.1`: close `PS-018` by expanding the proved emitted corpus, but keep
  `PS-007`, `PS-019`, and `PS-031` explicit external boundaries rather than
  silently folding runtime/spec obligations into emitted-package proof.
