...
- `integer` output is normalized without Ada `'Image` leading spaces,
  ...
  contents directly.
- The emitter lowers `print` calls through a generated `safe_io.ads`
  wrapper package marked `SPARK_Mode => Off`. This isolates the I/O
  side effect from the proved code surface:
    - The emitter generates `safe_io.ads` and `safe_io.adb` alongside the
      package output when `print` is used. The wrapper contains a single
      `put_line` procedure that calls `Ada.Text_IO.Put_Line`.
    - The emitted package body `with`s `Safe_IO` (not `Ada.Text_IO`
      directly) and calls `Safe_IO.Put_Line` with appropriate `'Image`
      normalization for integer and boolean arguments.
    - GNATprove sees `SPARK_Mode => Off` on `Safe_IO` and trusts the
      wrapper without proof. All other code in the emitted package
...
- Generated `safe_io.ads` / `safe_io.adb` wrapper with `SPARK_Mode => Off`.
...
## PR11.8c.2: Package-Level Statements

Allow executable statements at package scope so Safe programs have a natural
entry point without requiring a handwritten Ada driver.

### Scope

- Executable statements are admitted at package scope after all declarations.
  They run at package elaboration, before any tasks start.
- Package-level statements use the same parsing, resolution, and lowering
  paths as function body statements — no new statement kinds.
- The emitter lowers package-level statements into the Ada package body's
  elaboration section (`begin ... end Package_Name;`).
- Tasks declared in the same package start after package-level statements
  complete, following Ada's standard elaboration order.
- The `package` declaration is optional. If the first significant token in
  a `.safe` file is not `package` or `with`, the file is treated as a
  packageless entry file. The compiler infers the package name from the
  filename (`hello.safe` becomes package `hello`). A packageless file is
  the program's entry point — its statements become the Ada `Main` procedure
  rather than package elaboration code.
- In a multi-file program, exactly one file may be packageless (the entry
  point). All other files must have explicit `package` declarations. The
  compiler rejects a build with zero or multiple packageless files when
  building a complete program.
- Library-only builds (no entry point) require every file to have a
  `package` declaration.
- The `safe build` wrapper detects the entry file automatically and uses it
  as the Ada main unit, or generates a minimal driver that `with`s the
  named package if all files have explicit `package` declarations.
...
- Parser change to accept files without a leading `package` declaration,
  inferring the package name from the filename.
- Resolver and lowering support for package-level statements in the
  package's visible scope.
- Emitter support for the Ada `begin` elaboration block in package bodies
  and for generating an Ada `procedure Main` from packageless entry files.
- Positive fixtures demonstrating package-level `print`, computation,
  pre-task initialization, and packageless entry files.
- A negative fixture confirming that files with both a `package` declaration
  and entry-point semantics are handled correctly (only packageless files
  are entry points).
- Updated tutorial and CLI tutorial showing the complete `safe build` flow
  from a single-file `print ("hello world")` to multi-file programs with
  an entry file and library packages.
