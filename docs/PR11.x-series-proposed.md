...
- `declare` blocks and `declare_expression` remain explicit in PR11.6.
...
only policy, and it is also the natural landing point for the deferred
concurrency-surface syntax from PR11.5.
...
This milestone also absorbs the deferred concurrency-surface proposals from
PR11.5:
...
- Task channel direction constraints and scoped-binding receive land alongside
  the concurrency proof expansion so their legality boundaries and proof
  coverage ship together rather than ahead of the admitted concurrency corpus.
...
| Default Capacity Policy | PR11.10 (default values + build config) |
| safe.toml (build configuration) | PR11.10 (first use: default capacity overrides) |
...
| Task Channel Direction Constraints | PR11.8b |
| Scoped-Binding `receive` | PR11.8b |
