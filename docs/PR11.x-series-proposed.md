...
- PR11.8h follows PR11.8g.3 (safe prove CLI command).
...
- string/growable channel elements remain deferred to `PR11.8g`
...
- The following items are intentionally deferred beyond the shipped
  PR11.8d/PR11.8d.1 surface: broader proof-fact exact-length narrowing,
  string/growable channel elements to `PR11.8g`, and string discriminants.
...
- String/growable-array channel elements remain on `PR11.8g`.
...
- A broader blocking proof corpus that covers the admitted emitted proof-bearing
  surface, not just the current named checkpoint subsets.
...
- Updated proof/docs language so emitted concurrency code is no longer carried
  with external `PS-007` / `PS-031` caveats for the admitted forms.
...
| PS-007 | Source-level `select ... or delay` semantics | PR11.8g.3 |
...
| PS-031 | Jorvik/Ravenscar runtime obligations | PR11.8g.3 |
...
| Item | Why deferred |
|------|-------------|
| String as channel element type | Channel elements must be value types; string heap model needs channel-copy semantics |
| Growable array as channel element type | Same as string |
