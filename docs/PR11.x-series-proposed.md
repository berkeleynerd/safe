...
any blocking proof manifest. The current `PR11.8i.1` checkpoint moves proof
inventory ownership into `scripts/_lib/proof_inventory.py` and makes both
`run_proofs.py` and `run_tests.py` enforce exhaustive coverage; final full-lane
reclosure still depends on closing the remaining `pr118d1_for_of_string_build`
proof hotspot.
