...
Absorb the remaining admitted-but-unproved fixtures into blocking proof
manifests so that every emitted Ada package for the admitted Safe surface
proves at silver/bronze level.

After PR11.8g.1's expansion and PR11.8g.2's runtime/channel closure, 42
admitted fixtures in `tests/positive/`, `tests/build/`, and
`tests/concurrency/` remain outside any blocking proof manifest. This
milestone closes that gap.

### Scope

- **Widen dead-initialization suppression**: the emitter's
  "skip default initializer when a dominating write exists" logic currently
  covers task/concurrency paths but misses some sequential fixtures
  (`pr115_case_terminator`, `pr115_var_basic`, and similar). Extend the
  suppression to all generated scratch locals where the first statement
  unconditionally assigns.
- **Fix elaboration ordering for mutual-family allocators**: emitted Ada
  for `pr118e1_mutual_family_build` calls an allocator helper before the
  helper body appears. Reorder the emitted declarations so the body
  precedes the first call site.
- **Absorb fixtures in batches**: try each uncovered fixture under
  `safe prove`, fix the emitter issue that prevents proving, and add it
  to the blocking manifest. Categories:
  - Sequential fixtures with flow warnings (dead-init suppression)
  - Heap-backed runtime fixtures (may need contract-level work)
  - Print-dependent fixtures (should prove with existing IO contracts)
  - Binary-arithmetic fixtures (known numeric proof-expansion debt)
  - Enum fixtures (should prove or need minor emitter fixes)
  - Mutual-family and ownership fixtures (elaboration ordering)
- **Remove the `pragma Assume` for sequential channels**: re-emit
  sequential-only single-slot channels as record-based rather than
  protected-type-based (Option A), eliminating the last trusted assumption
  for that surface.

### Deliverables

- Every admitted fixture in `tests/positive/`, `tests/build/`, and
  `tests/concurrency/` is either in a blocking proof manifest or is
  explicitly excluded with a named reason and milestone owner.
- No unnamed "not in any manifest" bucket remains.
- The `pragma Assume` on sequential channel receives is eliminated.

### Dependency

Follows PR11.8i. Must land before PR11.8j so that incremental build
operates on a fully proved emitted surface.
