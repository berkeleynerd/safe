...
- `PR11.8b` runs in parallel after `PR10.5` and `PR10.6` to close the currently
  accepted emitted concurrency gap before the value-type channel milestone and
  `PR11.9` freeze artifact contracts.
...
- `integer (A to B)` provides inline range constraints at binding sites for
  parameters, fields, and locals. `subtype` provides named range aliases:
...
- Wide-intermediate lifting (`Safe_Runtime.Wide_Integer`) is removed — the
  default integer is already 64-bit. Intermediate arithmetic stays within
  the native integer range; multiplication of two 64-bit values is the one
  case where overflow remains possible and must be proved or trapped.
...
- PS-028 (TBD-10) is resolved: there is one integer type with a minimum
  64-bit range.
- PS-030 (TBD-12) is deferred to PR11.8c (modular arithmetic).
...
accepted emitted concurrency fixtures that still compile but are not yet under
the all-proved-only policy, and it is also the natural landing point for the
deferred concurrency-surface syntax from PR11.5.
...
Complete the numeric-model follow-through after `PR11.8a` by admitting a single
explicit modular arithmetic surface rather than leaving wrapping behavior
implicit or platform-defined.

### Scope

- `modular (N)` is the sole modular integer kind, wrapping at `2^N` for the
  standard widths `8`, `16`, `32`, and `64`.
- Explicit conversion is required between `integer` and `modular`; there is no
  implicit mixing.
- Bitwise operations (`and`, `or`, `xor`, `not`, and shifts) are admitted on
  `modular` only.
- Modular operations carry no overflow proof obligations because wraparound is
  defined behavior.

### Deliverables

- `PS-030` is resolved with a stable wraps-at-`2^N` rule and explicit boundary
  conversion policy.
- Tests, samples, and emission rules admit the modular surface without silently
  broadening ordinary-integer semantics.
...
- `in` parameters borrow for both value and reference types. `in out`
  parameters borrow mutably.
...
- The admitted Safe source, grammar, and documentation remove explicit
  ...
  `.access` / `.Access`, explicit access parameter modes, explicit
  deallocation, and value-typed `null`. These remain emitter-internal Ada
...
- The prover no longer needs to reason about cross-task ownership transfer
  through channels.
- Channel direction constraints from `PR11.8b` remain valid and unchanged; they
...
parallel track after PR10.5 and PR10.6 and should complete before PR11.8g and
PR11.9 so artifact contracts freeze against a broader proved concurrency
surface rather than only the frozen PR10 subset.

**Value-type semantic reset after the numeric reset** — PR11.8c, PR11.8d, and
PR11.8e deliberately continue the `PR11.8` rethink into modular arithmetic, a
...
- `PR11.8c` through `PR11.8e`: keep the value-type transition bounded to the
  admitted modular, string, and inferred-reference surface; do not silently
