...
- PR11.10 follows PR11.9d.
- PR11.11 follows PR11.10.
...
Add `fair select` as an optional qualifier that rotates the starting arm
after each successful dispatch.

### Scope

- New source syntax: `fair select` (optional qualifier)
- Maintain a per-select `Next_Arm` index that rotates after each
  successful arm dispatch, starting from `Next_Arm` and wrapping modulo
  the arm count
- Unqualified `select` retains declaration-order priority
...
- Add a ghost sequence type to `Channel_State` tracking element ordering
...
Remove blocking `send` from the language. All sends must use `try_send`
with explicit full-channel handling. Tasks can only block on `receive`.

### Why

Receive-only blocking on an acyclic channel graph cannot deadlock — there
is no circular wait when all blocking is unidirectional. This eliminates a
class of deadlocks by construction and makes the deadlock-freedom proof a
static graph property checkable at compile time.
...
- `send ch, value` becomes a compile error. The compiler rejects it with
  a diagnostic directing the programmer to use `try_send`.
- `try_send ch, value, success` remains the only send form. The
...
Every existing `send ch, value` becomes `try_send ch, value, ok` with a
failure handler. The migration is mechanical. The intended Safe idiom uses
PR11.8k's `try` propagation:

```safe
function produce returns result of integer
   try_send data_ch, value, ok
   if not ok
...
- Deadlock freedom for acyclic receive-only channel graphs becomes a
  compile-time guarantee
- Partially resolves TBD-09 from the spec register

### Deliverables

- Frontend rejection of `send` with a diagnostic
- Migration of all existing channel fixtures from `send` to `try_send`
- Updated spec §4.4, tutorial, and concurrency contract documentation
- Negative fixture for `send` rejection
...
| PS-035 | Broader `select ... or delay` fairness/latency beyond the admitted polling contract | PR11.9a / PR11.9b |
