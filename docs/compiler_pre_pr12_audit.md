...
- The cleanup and closeout were combined only because the cleanup drained the
  ...
  precedent from Phase 1C, Phase 1D, and Phase 1E.
...
Checkpoint 1 covers `compiler_impl/src/safe_frontend-ada_emit-statements.adb`
through the clean top-level dispatcher seam before `when CM.Stmt_For =>`.
The seam is line 4950 in the live file, after the complete `Stmt_While` arm and
before the outsized `Stmt_For` arm. That is 45.5% by line count and keeps the
checkpoint on a reviewer-useful dispatcher boundary rather than inside a local
helper or nested case statement.

Checkpoint 1 dispatchers/functions examined:

- Helper clusters before `Render_Statements`: cleanup/type/static tracking
  (`Render_Active_Cleanup`, `Render_Current_Cleanup_Frame`,
  `Invalidate_Static_Length_Tracking`,
  `Invalidate_Mutated_Call_Actual_Lengths`); name/use scanners
  (`Expr_Uses_Name`, `Statement_Uses_Name`,
  `Statements_Overwrite_Name_Before_Read`); select/channel helpers
  (`Select_Dispatcher_Name`, `Render_Channel_Operation_Target`,
  `Append_Staged_Channel_Call`, `Append_Staged_Channel_Length_Reconcile`,
  `Append_Staged_Channel_Target_Adoption`); declaration/type helpers
  (`Collect_Bounded_String_Types`, `Collect_For_Of_Helper_Types`,
  `Collect_Owner_Access_Helper_Types`, `Render_Object_Decl_Text_Common`);
  loop/condition helpers (`Loop_Variant_Image`,
  `Render_Variant_While_Guard_Image`,
  `Append_Counted_While_Lower_Bound_Invariant`,
  `Render_Shared_Condition`, `Append_Shared_Condition_Declarations`,
  `Render_Shared_Condition_From_Image`); channel/call emitters
  (`Emit_Nonblocking_Send_Statement`, `Render_Heap_Channel_Copy_Expr`,
  `Emit_Call_Statement`, `Render_Call_From_Image`).
...
| `Raise_Unsupported` path | select precheck failures; unsupported select arity; unsupported statement fallback; post-render helper failures | invalid select arm; missing select channel arm; select without channel arm; multi-delay select; unsupported statement kind; unsupported assignment/static-length helper paths | Covered: direct raises at lines 8866, 8873, 8953, 8961, 9086-9091, 9246, 9390, 9418, and 9461. No dead-code-after-raise issue found; each direct raise is a fail-closed branch or helper exit. The `Stmt_Try_Send` resolver-boundary failure is `Raise_Internal` at line 8551, not `Raise_Unsupported`, and is covered in the channel row. The line 9085 formatting issue is non-behavioral and recorded as CPA-002. |
...
### CPA-009 - Misindented public-channel JSON continuations
...
### CPA-010 - Misindented fallback subprogram aggregate component
