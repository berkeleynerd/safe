...
| While-loop variant proof-expansion surface | `tests/build/pr1123b_while_variant_patterns_build.safe` | PR11.23b extends the derivable while-loop variant surface with positive `.length` guards and downward integer countdown guards, while keeping MIR validation and Ada emission in lockstep. The fixture covers plain and bounded string length decrease, `> 0` and `>= 1` guards, strict and inclusive literal countdowns, arbitrary literal lower bounds, strict and inclusive two-identifier countdown bounds, mutable right-bound identifiers, and literal-left mirror forms under the blocking emitted-proof lane. | yes | yes | yes | yes | yes | none | no |
