...
| Multiple independent accumulator proof-expansion surface | `tests/build/pr1123e_multi_accumulator_build.safe` | PR11.23e emits independent fail-closed loop invariants for multiple integer accumulators in a growable `for of` body. The fixture covers a static `count = count + 1` accumulator, a direct bounded loop-item `sum = sum + item` accumulator, and an intentionally skipped non-additive `max_seen = item` update. | yes | yes | yes | yes | yes | none | no |
