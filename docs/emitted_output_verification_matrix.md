...
| Conditional string-growth proof-expansion surface | `tests/build/pr1123d_conditional_string_growth_build.safe`, `tests/negative/neg_string_concat_type_mismatch.safe`, `tests/negative/neg_array_concat_fixed_array.safe` | PR11.23d admits the documented `&` concatenation operator at parser level, keeps `+` numeric-only, closes the resolver's non-string/non-array `&` fall-through, and adds a fail-closed bounded-string `for of` growth heuristic. The emitted invariant uses the maximum static append length across conditional branches (`"yes"` vs. `"no"`) plus bounded-string capacity facts so generated `To_Bounded` preconditions prove at level 2. | yes | yes | yes | yes | yes | none | no |
