...
source-level select semantics and Jorvik/Ravenscar runtime obligations still
remain outside direct emitted-package GNATprove proof.
...
| Value-type channel elements with heap-backed values | `tests/build/pr118g_string_channel_build.safe`, `tests/build/pr118g_growable_channel_build.safe`, `tests/build/pr118g_tuple_string_channel_build.safe`, `tests/build/pr118g_record_string_channel_build.safe`, `tests/build/pr118g_try_string_channel_build.safe` | PR11.8g.2 closes the heap-backed channel seam by staging clone/free work outside protected channel bodies. The emitted protected operations now do only shallow move/reset plus index/count arithmetic, and the direct sequential single-slot string/growable path uses synchronized runtime-visible model scalars plus a narrow receive-side equality bridge (`pragma Assume (actual_length = returned_length)`) to complete the caller-side length handoff. | yes | yes | yes | yes | yes | Jorvik/Ravenscar runtime scheduling remains outside direct GNATprove proof; see `PS-031` in [`docs/post_pr10_scope.md`](post_pr10_scope.md). The direct sequential single-slot string/growable subset also relies on the documented receive-side equality `pragma Assume` bridge in the emitted Ada. | no |
...
| PR11.8g.1 concurrency proof-expansion set | `tests/concurrency/pr118b1_partial_task_clauses.safe`, `tests/concurrency/pr118b1_scoped_receive.safe`, `tests/concurrency/pr118b1_scoped_try_receive.safe`, `tests/concurrency/pr118b1_transitive_local_task_clause.safe` | PR11.8g.1 extends the blocking emitted-proof lane to the proof-friendly post-PR11.8b concurrency surface while keeping source-level fairness, latency, and Jorvik/Ravenscar obligations out at `PS-007` / `PS-031`. | yes | yes | yes | yes | yes | source/runtime concurrency semantics still remain external to emitted-package proof; see `PS-007` and `PS-031` in [`docs/post_pr10_scope.md`](post_pr10_scope.md). | no |
...
| Heap-backed runtime-backed string/growable surface | `tests/positive/pr118d_bounded_string.safe`, `tests/positive/pr118d_character_quote_literal.safe`, `tests/positive/pr118d_growable_array.safe`, `tests/positive/pr118d_string_length_attribute.safe`, `tests/positive/pr118d_string_mutable_object.safe`, `tests/build/pr118d1_growable_to_fixed_guard_build.safe`, `tests/build/pr118d_bounded_string_build.safe`, `tests/build/pr118d_bounded_string_field_build.safe`, `tests/build/pr118d_fixed_to_growable_build.safe`, `tests/build/pr118d_growable_to_fixed_literal_build.safe`, `tests/build/pr118d_growable_to_fixed_slice_build.safe` | PR11.8g.2 moves string/growable runtime support onto the shared proved runtime seam and brings the admitted heap-backed string/growable surface into the blocking emitted-proof lane. These fixtures now prove through the same shared runtime contract used by the rest of the emitted surface. | yes | yes | yes | yes | yes | none | no |
| Fixed-width binary emitted surface | `tests/positive/pr118c_binary_boolean_logic.safe`, `tests/positive/pr118c_binary_conversion_wrap.safe`, `tests/positive/pr118c_binary_inline_object.safe`, `tests/positive/pr118c_binary_named_type.safe`, `tests/positive/pr118c_binary_param_return.safe`, `tests/positive/pr118c_binary_shift.safe` | The blocking lane now covers the proved case-dispatch representative, but the remaining fixed-width binary outliers still need targeted proof or emit cleanup before they can be absorbed. They are no longer hidden inside a generic catch-all row; they remain explicit post-PR11.8g.1 expansion debt within the emitted numeric surface. | yes | yes | yes | no | no | explicit emitted numeric proof-expansion debt; see `PS-026` in [docs/PR11.x-series-proposed.md](PR11.x-series-proposed.md) | yes |
...
semantics and Jorvik/Ravenscar runtime scheduling/locking obligations remain
outside direct emitted-package GNATprove proof as `PS-007` and `PS-031` in
[`docs/post_pr10_scope.md`](post_pr10_scope.md).
