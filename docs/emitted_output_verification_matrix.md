...
| Value-type channel elements with heap-backed values | `tests/build/pr118g_string_channel_build.safe`, `tests/build/pr118g_growable_channel_build.safe`, `tests/build/pr118g_tuple_string_channel_build.safe`, `tests/build/pr118g_record_string_channel_build.safe`, `tests/build/pr118g_try_string_channel_build.safe` | PR11.8g.2 closes the heap-backed channel seam by staging clone/free work outside protected channel bodies. The emitted protected operations now do only shallow move/reset plus index/count arithmetic, and the direct sequential single-slot string/growable path uses synchronized runtime-visible model scalars plus a narrow receive-side equality bridge (`pragma Assume (actual_length = returned_length)`) to complete the caller-side length handoff. | yes | yes | yes | yes | yes | Blocking STM32F4/Jorvik Renode evidence covers the admitted runtime contract for the shipped channel subset. The direct sequential single-slot string/growable subset also relies on the documented receive-side equality `pragma Assume` bridge in the emitted Ada. Broader timing/fairness claims remain outside the admitted surface in [`docs/jorvik_concurrency_contract.md`](jorvik_concurrency_contract.md). | no |
| Select-with-delay emitted polling subset | `tests/concurrency/select_with_delay.safe`, `tests/concurrency/select_with_delay_multiarm.safe` | Frozen PR10 coverage proves one receive arm plus one delay arm, and supplemental hardening additionally proves a two-channel-arm success-path variant. Both are proved through the emitted fixed-quantum polling lowering used by the shipped implementation. | yes | yes | yes | yes | yes | Blocking STM32F4/Jorvik Renode evidence covers the admitted source-order select contract and its 1 ms polling-based delay behavior; broader fairness/latency claims remain outside the admitted surface in [`docs/jorvik_concurrency_contract.md`](jorvik_concurrency_contract.md). | no |
