...
| Built-in container checkpoint surface | `tests/positive/pr1110a_optional_guarded.safe`, `tests/positive/pr1110b_list_basics.safe`, `tests/positive/pr1110c_map_basics.safe`, plus the build-backed `pr1110a*`, `pr1110b*`, and `pr1110c*` fixtures in `scripts/_lib/proof_inventory.py` | PR11.10a/PR11.10b/PR11.10c close the shipped optional/list/map surface under blocking emitted proof manifests, and PR11.10d ratifies that union as the parent container checkpoint. `tests/build/pr1110b_list_empty_build.safe` now proves through the emitted lane; the remaining map-entry accumulator loop gap is tracked separately by PR11.22h.1. | yes | yes | yes | yes | yes | `tests/build/pr213_map_entry_build.safe` remains a named PR11.22h.1 runtime proof-gap target | no |
...
narrow warning suppressions for GNATprove's static-length false positives. The
remaining container-family runtime proof gap is
`tests/build/pr213_map_entry_build.safe`, whose map-entry `for of` loop still
needs generated accumulator facts for overflow proof and remains tracked by
PR11.22h.1 rather than the PR11.10d parent checkpoint.
