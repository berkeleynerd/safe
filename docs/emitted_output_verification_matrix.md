...
PR10 proof representatives: `tests/positive/rule2_binary_search.safe`,
`tests/positive/rule3_average.safe`, `tests/positive/rule4_linked_list.safe`,
`tests/positive/rule5_normalize.safe`, and `tests/positive/channel_pipeline.safe`.
...
| Post-PR10 ownership proof-expansion set | `tests/positive/ownership_borrow.safe`, `tests/positive/ownership_observe.safe`, `tests/positive/ownership_observe_access.safe`, `tests/positive/ownership_return.safe`, `tests/positive/ownership_inout.safe`, `tests/positive/ownership_early_return.safe` | Accepted, emitted, and compile-validated ownership fixtures beyond the frozen PR10 `ownership_move` representative. This six-fixture set is the first concrete required target of `PR10.3`. | yes | yes | yes | no | no | frontend Silver ownership analysis still governs legality, while emitted proof coverage for this expanded set is tracked by `PR10.3` in [`docs/pr10_refinement_audit.md`](pr10_refinement_audit.md). | yes |
...
| Other currently emitted sequential fixtures outside the PR10 corpus | remaining PR09 and PR08 accepted sequential subset beyond the ownership set above | Additional accepted sequential emission beyond both the frozen PR10 representatives and the named ownership expansion set remains outside current proved coverage. Further expansion stays under tracked task `PR10.3` in [`docs/pr10_refinement_audit.md`](pr10_refinement_audit.md). | yes | yes | yes | no | no | none | yes |
