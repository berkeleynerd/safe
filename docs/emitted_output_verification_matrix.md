...
| Value-type channel elements with heap-backed values | `tests/build/pr118g_string_channel_build.safe`, `tests/build/pr118g_growable_channel_build.safe`, `tests/build/pr118g_tuple_string_channel_build.safe`, `tests/build/pr118g_record_string_channel_build.safe`, `tests/build/pr118g_try_string_channel_build.safe` | PR11.8g.2 closes the heap-backed channel seam by staging clone/free work outside protected channel bodies. The emitted protected operations now do only shallow move/reset plus index/count arithmetic, and the direct sequential single-slot string/growable path uses synchronized Ghost model scalars plus a narrow receive-side equality bridge (`pragma Assume (actual_length = returned_length)`) to complete the caller-side length handoff. | yes | yes | yes | yes | yes | Jorvik/Ravenscar runtime scheduling remains outside direct GNATprove proof; see `PS-031` in [`docs/post_pr10_scope.md`](post_pr10_scope.md). The direct sequential single-slot string/growable subset also relies on the documented receive-side equality `pragma Assume` bridge in the emitted Ada. | no |
