#!/usr/bin/env python3
"""Run the PR11.4 signature/control-flow syntax cutover gate."""
...
from _lib.pr114_surface import (
    corpus_paths,
    negative_cases,
    negative_paths,
...
DEFAULT_REPORT = (
    REPO_ROOT / "execution" / "reports" / "pr114-signature-control-flow-syntax-report.json"
)

LEGACY_SOURCE_CHECKS = (
    (re.compile(r"\bprocedure\b"), "legacy `procedure`"),
    (re.compile(r"\belsif\b"), "legacy `elsif`"),
    (re.compile(r"\.\."), "legacy `..`"),
    (re.compile(r"\)\s*return\b|\bfunction\s+[A-Za-z_]\w*\s+return\b"), "legacy signature `return`"),
)


def assert_cutover_source(source: Path, expected_fragments: tuple[str, ...]) -> list[str]:
    raw_text = source.read_text(encoding="utf-8")
    normalized = normalize_source_text(raw_text)
    for fragment in expected_fragments:
        require(fragment in normalized, f"{source.name}: source is missing {fragment!r}")

    code_only = normalize_source_text(strip_safe_comments(raw_text))
    for pattern, label in LEGACY_SOURCE_CHECKS:
        require(not pattern.search(code_only), f"{source.name}: source still contains {label}")
...
def run_positive_case(
    *,
    ...
    source_fragments = assert_cutover_source(source, expected_source_fragments)
...
def generate_report(*, env: dict[str, str], scratch_root: Path | None = None) -> dict[str, Any]:
        ...
        for case in positive_cases():
            positives.append(
                run_positive_case(
                    ...
                    expected_ada_snippets=tuple(case["ada_snippets"]),
                )
            )
    ...
    return {
        ...
        "cutover_policy": {
            "coexistence": False,
            "full_quartet_scope": True,
            "accepted_source_spellings": ["function", "returns", "else if", "to"],
            "rejected_legacy_spellings": ["procedure", "return", "elsif", ".."],
        },
...
def main() -> int:
    ...
    print(
        "PR11.4 syntax cutover report written to"
...
if __name__ == "__main__":
    try:
        ...
        print(f"pr11.4 syntax cutover: ERROR: {exc}", file=sys.stderr)
        raise SystemExit(1)
