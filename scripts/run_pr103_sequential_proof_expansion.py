#!/usr/bin/env python3
"""Run the PR10.3 ownership proof-expansion milestone gate."""
...
def verify_corpus_contract(corpus: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    require(
        list(PR103_OWNERSHIP_PROOF_CASES) == expected,
        "PR10.3 ownership proof corpus must remain the exact six-fixture set",
    )
    return {
        "fixtures": expected,
    }
...
def main() -> int:
    ...
    print(f"pr103 sequential proof expansion: OK ({display_path(args.report, repo_root=REPO_ROOT)})")
    return 0
...
if __name__ == "__main__":
    try:
        ...
        print(f"pr103 sequential proof expansion: ERROR: {exc}", file=sys.stderr)
        raise SystemExit(1)
