...
package body Safe_Array_RT is
   ...
   begin
      ...
      for Index in Value'Range loop
         Result.Data (Target_Index) := Clone_Element (Value (Index));
   ...
   begin
      ...
      for Index in Positive (Low) .. Positive (High) loop
         Offset := Offset + 1;
         Result.Data (Positive (Offset)) := Clone_Element (Value.Data (Index));
      end loop;
      return Result;
   end Slice;
   ...
   begin
      ...
      if Left.Data /= null then
         for Index in Left.Data'Range loop
            Offset := Offset + 1;
            Result.Data (Positive (Offset)) := Clone_Element (Left.Data (Index));
         end loop;
      end if;
      if Right.Data /= null then
         for Index in Right.Data'Range loop
            Offset := Offset + 1;
            Result.Data (Positive (Offset)) := Clone_Element (Right.Data (Index));
         end loop;
      end if;
      return Result;
   end Concat;
