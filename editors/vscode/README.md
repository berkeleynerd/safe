...
through the current shipped PR11.8i compiler surface.
...
- The grammar statically highlights the current shipped lowercase surface,
  ...
  PR11.8c.1 statement-only `print` builtin, the PR11.8h `safe prove` tooling
  surface reflected in diagnostics workflows, and the PR11.8i
  identifier-valued user-defined enum declarations with `.first` / `.last`
  use sites. Mixed-case Safe spellings are highlighted as invalid.
...
- User-defined enum literals are highlighted as ordinary lowercase identifiers.
  This extension is syntax-only; it does not try to semantically distinguish
  enum literals from variables or imported constants.
