...
is
   ...
   --    1. Lift each Sensor_Value operand to Long_Long_Integer
   --    2. Accumulate in Long_Long_Integer (no overflow possible)
   --    3. Divide (by nonzero literal 10)
   --    4. Narrow the result at the return point
   -------------------------------------------------------------------
   function Average (Data : Sensor_Array) return Sensor_Value is
      --  Wide intermediate: sum computed in Long_Long_Integer.
      Sum : Long_Long_Integer := 0;
   begin
      for I in 1 .. 10 loop
         --  Loop invariant: Sum tracks accumulated values.
         --  Each Data element is in 0..1000, so after I iterations
         --  Sum is in 0 .. I * 1000.
         pragma Loop_Invariant (Sum >= 0);
         pragma Loop_Invariant (Sum <= Long_Long_Integer (I - 1) * 1000);

         --  Lift Data(I) to wide intermediate and accumulate.
         Sum := Sum + Data (I);
      end loop;
      ...
      begin
         pragma Assert (Wide_Result >= 0 and then Wide_Result <= 1000);

         --  Narrowing point (return): invoke Narrow_Return PO hook.
   ...
   --
   --  Emission pattern:
   --    1. Lift A and B to Long_Long_Integer
   ...
   is
      --  Wide intermediate computation.
      Wide_Sum : constant Long_Long_Integer :=
        Long_Long_Integer (A) + Long_Long_Integer (B);
   begin
      --  Narrowing point (assignment): invoke Narrow_Assignment PO hook.
